...
{
	name = "[CS:GO] Quests System - Gamble Addict", 
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("gambleaddict", "Gamble Addict", "Gamble on {progress} credits.", QuestType_Daily, 20000, 200000, 250000);
	}
}

public void Shop_OnClientGamble(int client, int credits, bool result, bool xgamble)
{
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(10000, 13000));
	}
} 