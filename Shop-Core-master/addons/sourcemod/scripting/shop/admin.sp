...
void Admin_OnSettingsLoad(KeyValues kv)
{
	...
	if (!count_menu.ItemCount)
	{
		...
		count_menu.AddItem("10000", "10,000 Credits");
		count_menu.AddItem("100000", "100,000 Credits");
	...
	switch (action)
	{
		case MenuAction_Display : 
		{
			...
			switch (g_iOpt[param1].AdminOption)
			{
				case GIVE_CREDITS : 
				{
					menu.SetTitle("%T\n• %N (%s)\n ", "give_credits_menu_title", param1, target, AddCommas(GetCredits(target)));
				}
				case TAKE_CREDITS : 
				{
					menu.SetTitle("%T\n• %N (%s)\n ", "take_credits_menu_title", param1, target, AddCommas(GetCredits(target)));
				}
				case SET_CREDITS : 
				{
					menu.SetTitle("%T\n• %N (%s)\n ", "set_credits_menu_title", param1, target, AddCommas(GetCredits(target)));
				}
			}
		}
...
void Admin_ShowMenu(int client, int pos = 0)
{
	...
	char title[128];
	FormatEx(title, sizeof(title), "%t", "admin_panel_menu_title");
	...
	FormatEx(buffer, sizeof(buffer), "%t", "give_credits_menu_button");
	menu.AddItem("a", buffer);
	FormatEx(buffer, sizeof(buffer), "%t", "take_credits_menu_button");
	menu.AddItem("b", buffer);
	FormatEx(buffer, sizeof(buffer), "%t\n ", "set_credits_menu_button");
	menu.AddItem("c", buffer);
	FormatEx(buffer, sizeof(buffer), "%t", "give_items_menu_button");
	menu.AddItem("d", buffer);
	FormatEx(buffer, sizeof(buffer), "%t", "take_items_menu_button");
	...
	switch (action)
	{
		case MenuAction_Display : 
		{
			switch (g_iOpt[param1].AdminOption)
			{
				case GIVE_ITEMS : 
				{
					menu.SetTitle("%t\n ", "give_items_menu_title");
				}
				case TAKE_ITEMS : 
				{
					menu.SetTitle("%t\n ", "take_items_menu_title");
				}
			}
		}
