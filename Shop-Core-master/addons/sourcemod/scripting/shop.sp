...
public int InfoHandle(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case MenuAction_Cancel : HideHud(param1, false);
		case MenuAction_Select : 
		{
			HideHud(param1, false);
...
public Action OnClientSayCommand(int client, const char[] command, const char[] sArgs)
{
	if (client > 0 && client <= MaxClients)
	{
		...
		if (Functions_OnClientSayCommand(client, szText) != Plugin_Continue)
		{
...
void ShowMainMenu(int client, int pos = 0)
{
	...
	char sBuffer[192];
	FormatEx(sBuffer, sizeof(sBuffer), "%T\n%T", "MainMenuTitle", client, "credits", client, AddCommas(PlayerManager_GetCredits(client)));
	...
	FormatEx(sBuffer, sizeof(sBuffer), "%T\n ", "inventory_menu_button", client);
	menu.AddItem("2", sBuffer);
	
	FormatEx(sBuffer, sizeof(sBuffer), "%T%s", "functions_menu_item", client, IsClientAllowed(client) ? "\n " : "");
	menu.AddItem("3", sBuffer);
	
	if (panel_info != null)
	{
		FormatEx(sBuffer, sizeof(sBuffer), "%T%s", "info", client, IsClientAllowed(client) ? "\n " : "");
		menu.AddItem("4", sBuffer);
	}
	
	if (IsClientAllowed(client))
	{
		FormatEx(sBuffer, sizeof(sBuffer), "%T", "admin_panel_menu_item", client);
...
public int MainMenu_Handler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		...
		case MenuAction_Cancel : HideHud(param1, false); // Hide the hud
		case MenuAction_Select : 
		{
			char info[4];
...
bool ShowInventory(int client)
{
	...
	char title[128];
	FormatEx(title, sizeof(title), "%T\n%T", "inventory_menu_title", client, "credits", client, AddCommas(PlayerManager_GetCredits(client)));
...
bool ShowCategories(int client)
{
	...
	char title[128];
	FormatEx(title, sizeof(title), "%T\n%T", "Shop", client, "credits", client, AddCommas(PlayerManager_GetCredits(client)));
...
bool ShowItemsOfCategory(int client, int category_id, bool inventory, int pos = 0)
{
	...
	// Get the viewed category name
	char category_name[SHOP_MAX_STRING_LENGTH];
	Shop_GetCategoryNameById(view_as<CategoryId>(category_id), category_name, sizeof(category_name));
	
	char title[128];
	if (inventory)
	{
		FormatEx(title, sizeof(title), "%T\n%T", "inventory_menu_title", client, "category", client, category_name);
		OnMenuTitle(client, Menu_Inventory, title, title, sizeof(title));
		iClMenuId[client] = Menu_Inventory;
	}
	else
	{
		FormatEx(title, sizeof(title), "%T\n%T", "shop_viewing_category", client, category_name, "credits", client, AddCommas(PlayerManager_GetCredits(client)));
		OnMenuTitle(client, Menu_Buy, title, title, sizeof(title));
		iClMenuId[client] = Menu_Buy;
	}
		...
		case MenuAction_DisplayItem:
		{
			...
			switch (ItemManager_GetItemTypeEx(info))
			{
				case Item_Finite : 
				{
					...
					{
						Format(sBuffer, sizeof(sBuffer), "[%s] %s (%d)", AddCommas(ItemManager_GetItemPriceEx(info)), sBuffer, PlayerManager_GetItemCountEx(param1, info));
					}
					result = true;
				}
				case Item_BuyOnly : 
				{
					if (!bInv[param1])
					{
						Format(sBuffer, sizeof(sBuffer), "[%s] %s", AddCommas(ItemManager_GetItemPriceEx(info)), sBuffer);
						result = true;
					}
				}
				default:
...
bool ShowItemInfo(int client, int item_id)
{
	...
	if (panel != null)
	{
		...
		FormatEx(sBuffer, sizeof(sBuffer), "%t\n%t\n ", "item_info_menu_title", "credits", AddCommas(credits));
			...
			case Item_None : 
			{
				if (PlayerManager_ClientHasItemEx(client, sItemId))
				{
					...
					if (timeleft > 0)
					{
						...
						if (sell_price > -1)
						{
							FormatEx(sBuffer, sizeof(sBuffer), "%t: %s", "absolute_sellprice", AddCommas(sell_price));
							panel.DrawText(sBuffer);
						}
					...
					if (sell_price > -1)
					{
						FormatEx(sBuffer, sizeof(sBuffer), "%t [+%s]", "sell", AddCommas(sell_price));
					...
					if (!isHidden)
					{
						switch (g_iItemTransfer)
						{
							...
							default : 
							{
								FormatEx(sBuffer, sizeof(sBuffer), "%t [%t: %s]", "transfer", "Costs", AddCommas(g_iItemTransfer));
...
bool SetCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		CPrintToChat(client, "%t", "set_you_credits", AddCommas(credits));
	}
...
int RemoveCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		CPrintToChat(client, "%t", "take_you_credits", AddCommas(credits));
	}
...
int GiveCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		CPrintToChat(client, "%t", "give_you_credits", AddCommas(credits));
	}
...
bool IsClientAllowed(int client)
{
	...
	{
		if (StrEqual(szGroup, g_szAuthorizedGroups[iCurrentGroup]))
		{
			return true;
		}
	}
...
void HideHud(int client, bool state)
{
	static ConVar cvDisableRadar;
	
	if (cvDisableRadar == null) {
		cvDisableRadar = FindConVar("sv_disable_radar");
	}
	
	if (cvDisableRadar != null && IsClientInGame(client)) {
		cvDisableRadar.ReplicateToClient(client, state ? "1":"0");
	}
} 