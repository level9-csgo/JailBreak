...
{
	new Handle:hVersion = CreateConVar("sm_pong_version", PLUGIN_VERSION, "Pong version", FCVAR_SPONLY | FCVAR_REPLICATED | FCVAR_NOTIFY | FCVAR_DONTRECORD);
	if (hVersion != INVALID_HANDLE)
		SetConVarString(hVersion, PLUGIN_VERSION);
	
	g_hCVOnlyDead = CreateConVar("sm_pong_onlydead", "1", "Allow only dead or spectating people to player pong.", _, true, 0.0, true, 1.0);
...
public Action Command_Pong(client, args)
{
	...
	if (g_iPongGameSession[client] == -1)
	{
		if (GetConVarBool(g_hCVOnlyDead) && IsPlayerAlive(client))
		{
			ReplyToCommand(client, "%sYou have to be dead to play Pong.", PREFIX);
			return Plugin_Handled;
		}
		
		Menu menu = new Menu(Menu_StartPong);
		
		menu.SetTitle("%s Pong Game - Main Menu\n ", PREFIX_MENU);
		menu.AddItem("chooseopponent", "Play Against A Player");
		menu.AddItem("computer", "Play Against The Computer\n \n◾ Use your walking keys to move up and down.\n◾ Type \"/pquit\" to quit a pong game.");
...
public Menu_StartPong(Menu menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		new bool:bOnlyDead = GetConVarBool(g_hCVOnlyDead);
		
		if (bOnlyDead && IsPlayerAlive(param1))
		{
			ReplyToCommand(param1, "%sYou have to be dead to play Pong.", PREFIX);
			return;
		}
		...
		if (StrEqual(info, "chooseopponent"))
		{
			...
			SetMenuTitle(menu2, "%s Pong Game -  Choose Opponent\n ", PREFIX_MENU);
...
public Menu_SelectPlayer(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		else if (bOnlyDead && IsPlayerAlive(param1))
		{
			PrintToChat(param1, "%sYou have to be dead or in spectator.", PREFIX);
		}
		else if (bOnlyDead && IsPlayerAlive(iTarget))
		{
		...
		else
		{
			...
			SetMenuTitle(menu2, "%s %N wants to play Pong with you!\n ", PREFIX_MENU, param1);
...
public Action:Timer_DrawPong(Handle:timer, any:iPongGameSession)
{
		 ...
		 && g_iBallPosition[iPongGameSession][1] != g_iPadPosition[iLeftPlayer])
	{
		...
		if (iRightPlayer != -1)
		{
			...
			PrintCenterText(iLeftPlayer, "You loose. %N won!", iRightPlayer);
			PrintToChat(iRightPlayer, "%sYou won. %N loose!", PREFIX, iLeftPlayer);
			PrintCenterText(iRightPlayer, "You won. %N loose!", iLeftPlayer);
		...
		else
		{
			...
			PrintCenterText(iLeftPlayer, "You loose. Computer won!");
		}
		 ...
		 && g_iBallPosition[iPongGameSession][1] != g_iPadPosition[iRightPlayer])
	{
		...
		PrintCenterText(iRightPlayer, "You loose. %N won!", iLeftPlayer);
		PrintToChat(iLeftPlayer, "%sYou won. %N loose!", PREFIX, iRightPlayer);
		PrintCenterText(iLeftPlayer, "You won. %N loose!", iRightPlayer);
