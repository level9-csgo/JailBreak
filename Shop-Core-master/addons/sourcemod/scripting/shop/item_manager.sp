...
void ItemManager_UnregisterMe(Handle plugin = null, bool plugin_end = false)
{
	...
	do
	{
		...
		delete view_as<DataPack>(h_KvItems.GetNum("callbacks", 0));
		...
		for (int category_id = 0; category_id < h_arCategories.Length; category_id++)
		{
			...
			if (index == -1)
			{
				if (plugin == null)
				{
					...
					{
						delete view_as<DataPack>(array.Get(j));
					}
			...
			delete view_as<DataPack>(array.Get(index));
...
Panel ItemManager_CreateItemPanelInfo(int source_client, int item_id, ShopMenu menu_act)
{
	...
	if (price < 1)
		...
		FormatEx(buffer, sizeof(buffer), "%t: %s", "Price", AddCommas(price));
	panel.DrawText(buffer);
	
	if (sell_price < 0)
		FormatEx(buffer, sizeof(buffer), "%t: %t", "Sell Price", "Unsaleable");
	else
		FormatEx(buffer, sizeof(buffer), "%t: %s", "Sell Price", AddCommas(sell_price));
