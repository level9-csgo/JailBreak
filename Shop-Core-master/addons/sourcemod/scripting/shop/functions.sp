...
void Functions_OnPluginStart()
{
	...
	g_hLuckCredits = CreateConVar("sm_shop_luck_credits", "50000", "How many credits the luck cost", 0, true, 0.0);
	g_hLuckChance = CreateConVar("sm_shop_luck_chance", "15", "How many chance the luck can be succeded", 0, true, 1.0, true, 100.0);
}
...
void Functions_ShowMenu(int client, int pos = 0)
{
	...
	char buffer[128];
	FormatEx(buffer, sizeof(buffer), "%t\n%t", "functions_menu_title", "credits", AddCommas(credits));
	...
	{
		FormatEx(buffer, sizeof(buffer), "%t %t", "trans_credits_menu_item", "trans_credits_disabled");
		menu.AddItem("a", buffer, ITEMDRAW_DISABLED);
	}
	else if (!g_iTransCredits)
	{
		FormatEx(buffer, sizeof(buffer), "%t", "trans_credits_menu_item");
		menu.AddItem("a", buffer);
	}
	else if (!g_bTransMode)
	{
		FormatEx(buffer, sizeof(buffer), "%t %t", "trans_credits_menu_item", "trans_credits_commision", g_iTransCredits);
		menu.AddItem("a", buffer);
	}
	else
	{
		FormatEx(buffer, sizeof(buffer), "%t %t", "trans_credits_menu_item", "trans_credits_cost", AddCommas(g_iTransCredits));
	...
	{
		FormatEx(buffer, sizeof(buffer), "%t %t", "try_luck", "luck_credits_chance", AddCommas(g_hLuckCredits.IntValue), g_hLuckChance.IntValue);
...
bool Functions_ShowCreditsTransferMenu(int client)
{
	...
	char buffer[128];
	FormatEx(buffer, sizeof(buffer), "%t\n%t", "trans_credits_menu_title", "credits", AddCommas(credits));
	OnMenuTitle(client, Menu_Functions, buffer, buffer, sizeof(buffer));
	menu.SetTitle(buffer);
	menu.ExitButton = true;
	menu.ExitBackButton = true;
...
void Functions_ShowPanel(int client)
{
	...
	FormatEx(buffer, sizeof(buffer), "%N (%s)", target, AddCommas(GetCredits(target)));
	...
	if (g_iCreditsTransferAmount[client] != 0)
	{
		...
		{
			FormatEx(buffer, sizeof(buffer), "%t", "credits_being_transfered", AddCommas(g_iCreditsTransferAmount[client]));
			panel.DrawText(buffer);
			
			if (g_bTransMode == false)
			{
				FormatEx(buffer, sizeof(buffer), "%t", "credits_to_transfer_commission", AddCommas(g_iCreditsTransferCommission[client]));
				panel.DrawText(buffer);
				
				FormatEx(buffer, sizeof(buffer), "%t", "credits_to_transfer", AddCommas(g_iCreditsTransferAmount[client] - g_iCreditsTransferCommission[client]));
				panel.DrawText(buffer);
			}
			else
			{
				FormatEx(buffer, sizeof(buffer), "%t", "credits_to_transfer_price", AddCommas(g_iCreditsTransferCommission[client]));
				panel.DrawText(buffer);
			}
		}
		else
		{
			FormatEx(buffer, sizeof(buffer), "%t", "credits_to_transfer", AddCommas(g_iCreditsTransferAmount[client]));
			panel.DrawText(buffer);
		}
		
		int left = credits - g_iCreditsTransferAmount[client] - g_iCreditsTransferCommission[client];
		
		FormatEx(buffer, sizeof(buffer), "%t", "transfer_credits_left", AddCommas(left));
		panel.DrawText(buffer);
		...
		if (left < 0)
		{
			FormatEx(buffer, sizeof(buffer), "%t", "need_positive", AddCommas(left * -1));
			panel.DrawText(buffer);
...
void Functions_SetupLuck(int client)
{
	...
	int credits = GetCredits(client);
	if (credits < g_hLuckCredits.IntValue)
	{
		// Exploiting bugs...
		Helpers_CloseHandleWithChatReason(hArray, client, "IncorrectCredits");
		return;
	}
...
stock bool Functions_AddTargetsToMenu(Menu menu, int filter_client)
{
	...
	{
		if (iCurrentClient != filter_client && IsAuthorizedIn(iCurrentClient))
		{
			IntToString(GetClientUserId(iCurrentClient), userid, sizeof(userid));
			FormatEx(buffer, sizeof(buffer), "%N (%s)", iCurrentClient, AddCommas(GetCredits(iCurrentClient)));
