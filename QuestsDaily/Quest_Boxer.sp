...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("boxer", "Boxer", "Kill {progress} prisoners while box is in-progress.", QuestType_Daily, 7500, 10, 20);
	}
}
...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (1 <= victim_index <= MaxClients && 1 <= attacker_index <= MaxClients && victim_index != attacker_index && GetClientTeam(victim_index) == CS_TEAM_T && JB_IsBoxEnabled())
	{
