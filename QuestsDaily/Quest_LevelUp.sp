...
{
	name = "[CS:GO] Quests System - Level Up", 
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("levelup", "Level Up", "Upgrade {progress} rune(s).", QuestType_Daily, 12500, 1, 3);
	}
}

public bool JB_OnRuneLevelUpgrade(int client, int clientRuneIndex, int targetLevel, bool &succeed)
{
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(8000, 10000));
	}
} 