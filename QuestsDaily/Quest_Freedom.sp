...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("freedom", "Freedom", "Choose Rebel from the last request selection menu.", QuestType_Daily, 8500, 1, 1);
	}
}
...
{
	// If the last request 
	if (JB_FindLr("Rebel [Rambo Mode]") == lrId)
	{
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(6000, 7000));
	}
} 