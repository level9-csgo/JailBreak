...
{
	name = "[CS:GO] Quests System - Revenge Kill", 
	author = "KoNLiG", 
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("revengekill", "Revenge Kill [Extreme]", "Kill one player while you're dead.", QuestType_Weekly, 95000, 1, 1);
	}
}
...
void Event_PlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
	...
	if ((1 <= attacker_index <= MaxClients) && attacker_index != GetClientOfUserId(event.GetInt("userid")) && !IsPlayerAlive(attacker_index))
	{
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(60000, 80000));
	}
} 