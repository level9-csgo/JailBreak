...
{
	name = "[CS:GO] Quests System - Last Request Winner", 
...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("lastrequestwinner", "Last Request Winner", "Win {progress} last requests.", QuestType_Daily, 15500, 10, 15);
	}
}

public void JB_OnLrEnd(int currentLr, const char[] lrName, int winner, int loser, bool aborted)
{
	// Make sure the last request has naturally finished, and the winner index is valid
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(5000, 10000));
	}
} 