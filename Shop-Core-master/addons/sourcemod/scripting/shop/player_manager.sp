...
public int PlayerManager_SetClientCredits(Handle plugin, int numParams)
{
	...
	int credits_amount = GetNativeCell(2);
	
	if (credits_amount < 0)
	{
		ThrowNativeError(SP_ERROR_NATIVE, "Credits amount to set must be over 0! (Got: %d)", credits_amount);
	}
	
	char file_name[64];
	GetPluginFilename(plugin, file_name, sizeof(file_name));
	WriteLogLine("Credits of \"%L\" has set to %s from %s, by plugin \"%s\".", client, AddCommas(credits_amount), AddCommas(GetCredits(client)), file_name);
	
	return PlayerManager_SetCredits(client, credits_amount);
}
...
public int PlayerManager_GiveClientCredits(Handle plugin, int numParams)
{
	...
	char file_name[64];
	GetPluginFilename(plugin, file_name, sizeof(file_name));
	WriteLogLine("%s credits has been given to \"%L\", by plugin \"%s\".", AddCommas(credits_amount), client, file_name);
	
	return GiveCredits(client, credits_amount, GetNativeCell(3));
}
...
public int PlayerManager_TakeClientCredits(Handle plugin, int numParams)
{
	...
	char file_name[64];
	GetPluginFilename(plugin, file_name, sizeof(file_name));
	WriteLogLine("%s credits has been taken from \"%L\", by plugin \"%s\".", AddCommas(credits_amount), client, file_name);
