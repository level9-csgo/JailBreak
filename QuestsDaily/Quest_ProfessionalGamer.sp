...
public void OnLibraryAdded(const char[] name)
{
	...
	{
		g_QuestIndex = JB_CreateQuest("professionalgamer", "Professional Gamer", "Win {progress} side games.", QuestType_Daily, 4500, 5, 15);
	}
}

public void JB_OnGameStop(int gameId, int winner)
{
	if (winner != INVALID_GAME_WINNER)
	{
		// Add one progress point to the side game winner
...
public void JB_OnQuestAssigned(int client, int quest_id, int target_progress, int &reward, ExecuteType execute_type)
{
	if (g_QuestIndex == quest_id && execute_type == ExecuteType_Pre)
	{
		reward = RoundToDivider(GetRandomInt(2000, 4000));
	}
} 