...
ConVar top_gang_wins_min_players;
ConVar top_gang_wins_min_gangs;
...
public void OnPluginStart()
{
	...
	top_gang_wins_min_players = CreateConVar("top_gang_wins_min_players", "6", "Minimum online players to enable top gang wins.", .hasMin = true, .min = 2.0, .hasMax = true, .max = float(MAXPLAYERS));
	top_gang_wins_min_gangs = CreateConVar("top_gang_wins_min_gangs", "2", "Minimum online individual gangs to enable top gang wins.");
...
{
	if (!IsTopGangWinsEnabled() || winner == INVALID_DAY_WINNER)
	{
		return;
	}
...
{
	GangEntry gang_entry;
	g_GangEntrys.GetArray(gang, gang_entry);
	strcopy(gang_entry.name, sizeof(gang_entry.name), newName);
	g_GangEntrys.SetArray(gang, gang_entry);
}
...
bool IsTopGangWinsEnabled()
{
	return GetClientCount() >= top_gang_wins_min_players.IntValue && GetOnlineGangCount() >= top_gang_wins_min_gangs.IntValue;
}

int GetOnlineGangCount()
{
	ArrayList online_gangs = new ArrayList();
	
	for (int current_client = 1, gang_id; current_client <= MaxClients; current_client++)
	{
		if (IsClientInGame(current_client) && (gang_id = Gangs_GetPlayerGang(current_client)) != -1 && online_gangs.FindValue(gang_id) == -1)
		{
			online_gangs.Push(gang_id);
		}
	}
	
	int online_gang_count = online_gangs.Length;
	
	delete online_gangs;
	
	return online_gang_count;
}
